...
contract TriaToken is ERC20, ERC20Permit, Ownable, Withdrawable {

    constructor(address owner, address treasury) ERC20("TRIA", "TRIA") ERC20Permit("TRIA") Ownable(owner) {
        _mint(treasury, 10_000_000_000 * 1e18);
    }

    function _authorizeWithdraw() internal override view returns (bool) {
        return msg.sender == owner();
    }

    function _getWithdrawTo() internal override view returns (address) {
        return owner();
    }
}
...
// File: contracts/base/Withdrawable.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.28;
...
abstract contract Withdrawable is ReentrancyGuard {
    ...
    error NotAuthorized();
    error InvalidWithdrawToAddress();
    error FailedToWithdraw();

    modifier onlyAuthorizer() {
        if (!_authorizeWithdraw()) revert NotAuthorized();
        _;
    }

    function withdraw() external onlyAuthorizer nonReentrant {
        address withdrawTo = _getWithdrawTo();
        if (withdrawTo == address(0)) revert InvalidWithdrawToAddress();

        uint256 balance = _getBalance();
        (bool success, ) = payable(withdrawTo).call{value: _getBalance()}("");
        if (!success) revert FailedToWithdraw();
        emit Withdrawn(withdrawTo, balance);
    }

    function withdrawToken(address token) external onlyAuthorizer nonReentrant {
        address withdrawTo = _getWithdrawTo();
        if (withdrawTo == address(0)) revert InvalidWithdrawToAddress();

        uint256 balance = _getTokenBalance(token);
        IERC20Metadata(token).safeTransfer(withdrawTo, balance);
        emit WithdrawnToken(token, withdrawTo, balance);
    }
    ...
    function _getTokenBalance(address token) internal view virtual returns (uint256) {
        return IERC20Metadata(token).balanceOf(address(this));
    }

    function _authorizeWithdraw() internal view virtual returns (bool);

    function _getWithdrawTo() internal view virtual returns (address);
}
