...
/**
 * @title BTR Token Contract
 * @dev ERC20 token with burn, permit, and ERC1363 support.
 * Implements minting and burning restrictions with role-based access control.
 */
contract BTR is Ownable2Step, ERC20Permit, ERC20Burnable, ERC1363 {
    using EnumerableSet for EnumerableSet.AddressSet;

    /// @notice Maximum token supply
    uint256 public immutable MAX_SUPPLY;

    /// @notice Mint quota limit
    ...
    /// @notice Event emitted when mint quota is updated
    event MintQuotaSet(uint256 mintQuota);
    ...
    constructor(
        ...
        address owner_,
        uint256 mintQuota_
    ) Ownable(owner_) ERC20Permit(name_) ERC20(name_, symbol_) {
        MAX_SUPPLY = 1_000_000_000 * (10 ** decimals_);
        mintQuota = mintQuota_;
        i_decimals = decimals_;
    }

    /**
     * @dev Updates the mint quota.
     * Only the contract owner can call this function.
     * @param mintQuota_ New minting quota
     */
    function setMinQuota(uint256 mintQuota_) public onlyOwner {
        require(mintQuota_ <= MAX_SUPPLY, "mintQuota exceeds max supply");
        require(
            mintQuota_ >= totalSupply(),
            "mintQuota less than total supply"
        );
        mintQuota = mintQuota_;
        emit MintQuotaSet(mintQuota_);
    }
    ...
    function mint(address to, uint256 amount) public onlyMinter {
        require(
            totalSupply() + amount <= mintQuota,
            "minting exceeds max quota"
        );
        _mint(to, amount);
    }
